...
namespace DigitalRuby.WeatherMaker
{
    ...
    public class WeatherMakerFullScreenCloudsScript : MonoBehaviour
    {
...
#if UNITY_URP
        private struct WeatherMapActionState
        {
            public CameraEvent Event;
            public System.Action<WeatherMakerCommandBufferContext> Action;
        }

        private readonly Dictionary<Camera, WeatherMapActionState> weatherMapActions =
            new Dictionary<Camera, WeatherMapActionState>();
#endif
        ...
        private void GenerateWeatherMap(Camera camera)
        {
            ...
            CameraEvent weatherMapEvent = WeatherMakerCommandBufferManagerScript.GetWeatherMapCameraEvent(camera);
            RemoveWeatherMapCameraWork(camera);
            ...
            if (WeatherMapRenderTexture != null)
            {
                ...
                AddWeatherMapCameraWork(camera, weatherMapEvent);

            }
        }

        private void RemoveWeatherMapCameraWork(Camera camera)
        {
            if (camera == null)
            {
                return;
            ...
            if (WeatherMakerCommandBufferManagerScript.Instance != null &&
                weatherMapActions.TryGetValue(camera, out var state))
            {
                WeatherMakerCommandBufferManagerScript.Instance.RemoveCameraCommandBufferAction(state.Action, camera, state.Event);
                weatherMapActions.Remove(camera);
            }
#else
            if (weatherMapCommandBuffer != null)
            {
        ...
        private void AddWeatherMapCameraWork(Camera camera, CameraEvent weatherMapEvent)
        {
            if (camera == null || weatherMapCommandBuffer == null)
            {
                return;
            }
            ...
            CommandBuffer capturedCmd = weatherMapCommandBuffer;
            System.Action<WeatherMakerCommandBufferContext> action = (ctx) =>
            {
                // Replay the pre-built weather map commands in the current URP pass.
                Graphics.ExecuteCommandBuffer(capturedCmd);
            };
            weatherMapActions[camera] = new WeatherMapActionState
            {
                Event = weatherMapEvent,
                Action = action
            };
            WeatherMakerCommandBufferManagerScript.Instance.AddCameraCommandBufferAction(action, camera, weatherMapEvent);
#else
            camera.AddCommandBuffer(weatherMapEvent, weatherMapCommandBuffer);
#endif
        }
        ...
        private void OnDisable()
        {
            ...
            if (WeatherMakerCommandBufferManagerScript.Instance != null)
            {
                foreach (var kv in weatherMapActions)
                {
                    if (kv.Key != null)
                    {
                        WeatherMakerCommandBufferManagerScript.Instance.RemoveCameraCommandBufferAction(
                            kv.Value.Action, kv.Key, kv.Value.Event);
                    }
                }
            }
            weatherMapActions.Clear();
#endif
        ...
        internal void SetShaderCloudParameters(Material material, Camera camera)
        {
            if (WeatherMakerLightManagerScript.Instance != null &&
                currentRenderCloudProfile != null &&
                material != null)
            {
        ...
        private bool PreSetupCommandBuffer(WeatherMakerCommandBuffer cmdBuffer)
        {
            ...
            if (WeatherMakerSkySphereScript.Instance != null)
            {
                ...
                WeatherMakerFullScreenEffect.SetCameraTarget(cmdBuffer.CommandBuffer);
